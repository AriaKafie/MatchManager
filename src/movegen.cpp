...
Move *generate_moves(const Position *pos, Move *list)
{
    ...
    Bitboard bishop_queen = pos->bb(FriendlyBishop) | pos->bb(FriendlyQueen);
    Bitboard rook_queen   = pos->bb(FriendlyRook)   | pos->bb(FriendlyQueen);
    ...
    *list = SCASTLE;
    list += !((seen_by_enemy & k_no_atk | occupied & k_no_occ | !pos->kingside_rights(Us)));

    *list = LCASTLE;
    list += !((seen_by_enemy & q_no_atk | occupied & q_no_occ | !pos->queenside_rights(Us)));
